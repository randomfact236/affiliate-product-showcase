...
export class MediaService {
  ...
  constructor(private readonly configService: ConfigService) {
    this.bucketName = this.configService.get('STORAGE_BUCKET') || 'affiliate-media';
    
    const endpoint = this.configService.get('STORAGE_ENDPOINT') || 'localhost';
    const port = this.configService.get('STORAGE_PORT') || '9000';
    const accessKey = this.configService.get('STORAGE_ACCESS_KEY') || 'minio';
    const secretKey = this.configService.get('STORAGE_SECRET_KEY') || 'minio_secret';
    const region = this.configService.get('STORAGE_REGION') || 'us-east-1';
    
    this.publicUrlBase = this.configService.get('STORAGE_PUBLIC_URL') || 
  ...
  async uploadFile(file: MulterFile): Promise<FileUploadResult> {
    ...
    if (file.mimetype !== detectedType && !this.isMimeTypeCompatible(file.mimetype, detectedType)) {
      throw new BadRequestException(
